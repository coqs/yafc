...
import { getFileSize, getFolderFiles, getFileName, extractZIP, getFolderRoots, hostFile } from "./fileSystem.js";
...
app.post("/hostFile", async (req, res) => {
    let filePath = req.body.filePath;
    let hostedFileURL = await hostFile(filePath);
    res.status(200);
    res.send(hostedFileURL);
});

app.post("/selectFolder", async (req, res) => {
    const result = await fileDialog({ type: 'directory' });
    if (result && result.length > 0) {
        res.send(result[0]);
    } else {
        res.status(400).send("no folder selected");
    }
});

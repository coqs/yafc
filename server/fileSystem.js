...
export const hostFile = async (filepath) => {
    const { stdout, stderr } = await execPromise(`curl -F "reqtype=fileupload" -F "time=1h" -F "fileToUpload=@${filepath}" https://litterbox.catbox.moe/resources/internals/api.php`);
    console.log(`hosted on ${stdout}`);
    return stdout;
}